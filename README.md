...
2. 粘贴网易云**公开**歌单链接（如 `https://music.163.com/#/playlist?id=XXXXX`）或纯数字歌单 ID。
...
- **只读公开数据**：本工具仅请求网易云的公开歌单元数据接口，不读取、不存储任何私人数据。
- **本工具不需要网易云账号或 Cookie**：正常使用全程无需登录。请不要在界面或配置中粘贴任何平台的 Cookie、登录态或私人令牌——本工具的任何环节都不会用到它们。
